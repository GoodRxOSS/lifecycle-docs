...
### Agent sessions and sandbox sessions

If an agent session needs a secret during workspace bootstrap, keep the secret
on the service's normal environment and allowlist the key in the service's
`dev.forwardEnvVarsToAgent` block.
...
This forwards only the listed keys into the per-session agent secret. Lifecycle
then injects those values into the workspace bootstrap and runtime containers.
It is useful for package registry tokens and other install-time secrets the
agent needs before dev mode starts.

Additional rules:

- If the forwarded value is a native secret reference such as `{{aws:...}}` or
  `{{gcp:...}}`, Lifecycle must have `secretProviders` configured.
- If multiple selected services forward the same key into one session, that key
  must resolve to the same value for every selected service.
