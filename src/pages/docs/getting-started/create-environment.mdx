...
<Callout type="info" icon={Info}>
  If a build or deployment fails, follow the Environment link and inspect the
  failed Service in Lifecycle. [Troubleshoot the failure from the Service
  summary and Console](/docs/troubleshooting/deploy-issues).
</Callout>
